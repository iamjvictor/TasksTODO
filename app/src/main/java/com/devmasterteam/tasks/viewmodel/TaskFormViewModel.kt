...
class TaskFormViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun loadPriorities(){
        ...
        val localData = priorityRepository.list()



        if (localData.isNotEmpty()) {
            // Se houver dados no banco de dados local, os utiliza
            _priorityList.value = localData
        } else {
            // Se não houver dados locais, tenta buscar da API
            priorityRepository.list(object : APIListener<List<PriorityModel>> {
                override fun onSuccess(result: List<PriorityModel>) {
                    _priorityList.value = result
                }
